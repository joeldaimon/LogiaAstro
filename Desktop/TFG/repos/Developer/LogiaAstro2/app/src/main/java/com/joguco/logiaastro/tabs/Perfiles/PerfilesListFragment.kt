...
class PerfilesListFragment : Fragment() {
                    ...
                    db.collection("users").get()
                        .addOnSuccessListener { documents ->
                            for (user in documents) {
                                ...
                                if(user.data.toString().contains(actualUser)){
                                    setFollowing(user.data)
                                }
                            }
                        }
    ...
    private fun setFollowing(data: Map<String, Any>) {
        data.forEach { entry ->
            when(entry.key) {
                "following" -> {//Nombre del Usuario
                    following = entry.value as ArrayList<String>
                }
            }
        }
    }
